...
func (c *Connection) SendBuffMsg(msg interface{}) error {
	c.RLock()
	defer c.RUnlock()
	if c.isClosed {
	...
	msgId := (*c.GetServer()).GetMsgId(msg)
	data, err := proto.Marshal(msg.(proto.Message))
	if err != nil {
		return errors.New("proto Marshal error:" + err.Error())
	}

	//将data封包，并且发送
	dp := c.TCPServer.Packet()
	msgBuff, err := dp.Pack(NewMsgPackage(msgId, data))
	if err != nil {
		fmt.Println("Pack error msg ID = ", msgId)
		return errors.New("Pack error msg ")
	}
