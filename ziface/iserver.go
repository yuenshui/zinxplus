...
type IServer interface {
	...
	Stop()                                     // 停止服务器方法
	Serve()                                    // 开启业务服务方法
	SetMsgMap(MsgMap *map[uint32]interface{})  // 注册消息列表
	GetMsgMap() *map[uint32]interface{}        // 获取消息列表
	GetMsgId(msg interface{}) uint32           // 获取消息的ID
	AddRouter(msg interface{}, router IRouter) // 路由功能：给当前服务注册一个路由业务方法，供客户端链接处理使用
